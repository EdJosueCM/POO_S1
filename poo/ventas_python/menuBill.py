...
class CrudClients(ICrud):
    ...
    def validar_cedula_decorator(func):
        def wrapper(self):
            while True:
                borrarPantalla()
                self.print_separator()
                user_ID = input("Ingrese el numero de cedula: ")
                if len(user_ID) != 10:
                    print("La cédula debe contener exactamente 10 dígitos. Vuelva a intentarlo.")
                    continuar = input("¿Desea continuar ingresando la cédula? (s/n): ")
    ...
    @validar_cedula_decorator
    def create(self, user_ID):
        while True:
            if not user_ID.isdigit():
                print("La cedula debe contener solo numeros.")
                continuar = input("¿Desea continuar ingresando clientes? (s/n): ")
                if continuar.lower() != "s":
                    break
                continue
            ...
            if any(client['dni'] == user_ID for client in clients):
                print("La cedula ya existe. Vuelva a intentarlo.")
                continuar = input("¿Desea continuar ingresando clientes? (s/n): ")
                if continuar.lower() != "s":
                    break
                continue
